...
Object.extend(Object.extend(Effect.Slideshow.prototype, Effect.Base.prototype), {
  ...
  _shared_initialize: function (options) {
    ...
    if (this.options.showEffect && this.options.hideEffect) {
      this._effectPair = { show: null, hide: null };
      for (var direction in this._effectPair) {
        var effect = this.options[direction + 'Effect'];
        if (typeof effect == 'function') {
          this._effectPair[direction] = effect;
        }
        else {
          this._effectPair[direction] = this._effectHandler[effect];
        }
      }
    }
      ...
      this._effectPair = this._buildEffectPair(Slideshow.Options.effectPairs[this.options.effectPair]);
    }
    else {
  ...
  _effectHandler: function (effect) {
    var name = effect;
    return function (element, options) {
      new Effect[name](element, options);
    };
  },
  _buildEffectPair: function (presets) {
    return { show: this._effectHandler(presets[0]), hide: this._effectHandler(presets[1]) };
  },
    ...
    if (newFrame) {
      if (this.options.effectPair == 'random') {
        ...
        if (this._unusedEffectPairs.length) {
          ...
          this._effectPair = this._buildEffectPair(Slideshow.Options.effectPairs[nextPair]);
        }
        else {
...
Object.extend(Object.extend(Ajax.Slideshow.prototype, Effect.Slideshow.prototype), {
  initialize: function (element, url, options) {
    ...
    if (options.beforeChange) {
      this._oldBeforeChange = options.beforeChange;
      options.beforeChange = function () { this._oldBeforeChange(this); this._requestFrames(); }.bind(this);
    }
    else {
      options.beforeChange = this._requestFrames.bind(this);
    }
    this._shared_initialize(options);
  },
...
Object.extend(Object.extend(Slideshow.Local.prototype, Effect.Slideshow.prototype), {
  initialize: function (element, frames, options) {
    ...
    options             = options || {};
    if (options.beforeChange) {
      var _oldBeforeChange = options.beforeChange;
      options.beforeChange = function () { _oldBeforeChange(this); this._addFrames(); }.bind(this);
    }
    else {
      options.beforeChange = this._addFrames.bind(this);
    }
    this._shared_initialize(options);
  },
  _addFrames: function () {
